...
## File-Per-Slice State Integration (C-81, C-82)

This section documents how the pipeline adapts its state read/write targets when the project uses file-per-slice format (C-80). Legacy format behaviour is completely unchanged (D-37) — if the state format is legacy, all reads and writes go to STATE.md as before.

### State Format Detection

Before Step 1, detect the state format (C-80):

- If file-per-slice: read the target slice file from `.greenlight/slices/{id}.md`
- If legacy: read STATE.md as before (no change)

Slice ID validated before file path construction to prevent path traversal attacks.

### Step 4 (Claim Slice / Start Implementation)

When using file-per-slice format, Step 4 must:

1. Write status + step to own slice file only — /gl:slice writes ONLY to its own slice file (never to another slice's file)
2. Set session field in slice file frontmatter (C-82) BEFORE any agent work begins
3. Regenerate STATE.md after writing to the slice file (D-34)
4. If legacy: write to STATE.md as before (no change)

Session field is set using the format: `{ISO timestamp}-{4-char hex suffix}` (example: `2026-02-22T14:00:00Z-a7f3`). The hex suffix is random. On SessionGenerationFailure (cannot generate random suffix), use timestamp-only as fallback and warn.

If the slice already has a session field set, this indicates a ConcurrentSliceClaim: Warn the user and prompt "Continue anyway? (y/n)". In yolo mode, skip the prompt and auto-continue with a warning log. Session tracking is advisory only (D-33) — it is never blocking.

Session tracking only applies in file-per-slice mode (not legacy).

### Step 10 (Completion / Status Update)

When using file-per-slice format, Step 10 must:

1. Write final status + test counts to own slice file
2. Clear session field (set to empty string) on successful completion
3. Regenerate STATE.md after writing to the slice file (D-34)
4. If legacy: write to STATE.md as before (no change)

### STATE.md Regeneration Protocol

After every write to a slice file:

1. Read all slice files from `.greenlight/slices/`
2. Read `project-state.json` for overview, session, and blockers
3. Write STATE.md with the GENERATED header:
   ```
   <!-- GENERATED by greenlight -- source of truth is .greenlight/slices/*.md -->
   ```
4. STATE.md is a convenience view / summary view — the slice file is the source of truth

Concurrent sessions writing to different slices never conflict (FR-2) because each session writes only to its own slice file.

### Crash Safety (NFR-4)

All state writes use write-to-temp-then-rename to prevent corruption:
...
If STATE.md regeneration fails (RegenerationFailure): Warn but continue. The slice file is still correct — partial failure is safe. The slice file remains the authoritative record.

### Error Handling

| Error State | When | Behaviour |
|-------------|------|-----------|
| SliceFileNotFound | Target slice file does not exist | Create it. Warn |
| SliceFileWriteFailure | Cannot write to slice file | Report error. Do not proceed |
| RegenerationFailure | Cannot regenerate STATE.md | Warn but continue. Slice file is still correct |
| ConcurrentSliceClaim | Target slice has active session | Warn and prompt |

### Session Tracking Error Handling (C-82)

| Error State | When | Behaviour |
|-------------|------|-----------|
| StaleSession | Session field has value but no active process | Warning only |
| SessionGenerationFailure | Cannot generate random suffix | Use timestamp-only fallback. Warn |

Stale sessions are warned but never auto-cleaned. Session field is preserved when a slice is paused — the next session gets an advisory warning.
...
- Slice file is the source of truth; STATE.md is a generated convenience view
